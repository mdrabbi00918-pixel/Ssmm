...
<div class="grid"><?php foreach($products as $p): ?><div class="card service-card"><div style="position:relative"><img loading="lazy" src="<?=e($p['image_url'] ?: 'data:image/svg+xml,%3Csvg xmlns=%22http://www.w3.org/2000/svg%22 width=%22600%22 height=%22300%22%3E%3Crect width=%22100%25%22 height=%22100%25%22 fill=%22%23101828%22/%3E%3Ctext x=%2250%25%22 y=%2250%25%22 dominant-baseline=%22middle%22 text-anchor=%22middle%22 fill=%22white%22 font-size=%2228%22%3ESupercell Item%3C/text%3E%3C/svg%3E' )?>" alt="<?=e($p['name'])?>" style="width:100%;height:190px;object-fit:cover;border-radius:14px"></div><h3><?=e($p['name'])?></h3><?php if($p['description']): ?><p class="muted"><?=e($p['description'])?></p><?php endif; ?><div class="bottom"><b class="price">৳<?=number_format((float)$p['price'],2)?><br><span style="font-size:.9em">$<?=number_format((float)($p['price_usd']??0),2)?></span></b><form method="post"><input type="hidden" name="csrf" value="<?=e(csrf())?>"><input type="hidden" name="action" value="buy_digital"><input type="hidden" name="product_id" value="<?=e((string)$p['id'])?>"><button class="btn" type="submit">💳 Buy & Get Link</button></form></div></div><?php endforeach; ?><?php if(!$products): ?><div class="card"><h3>এই মুহূর্তে কোনো item available নেই</h3><p class="muted">নতুন Supercell item যোগ হলে এখানে দেখা যাবে।</p></div><?php endif; ?></div>
<h3 class="section-title" id="purchase-history">👤 My Supercell Purchase History</h3><div class="grid"><?php if(!$owned): ?><div class="card"><p class="muted">আপনি এখনো কোনো Supercell item কিনেননি।</p></div><?php else: foreach($owned as $op): ?><div class="card service-card"><div class="muted">Purchased: <?=e($op['purchased_at'])?></div><h3><?=e($op['name'])?></h3><div class="bottom"><b class="price">৳<?=number_format((float)$op['price'],2)?><br><span style="font-size:.9em">$<?=number_format((float)($op['price_usd']??0),2)?></span></b><button class="btn green" type="button" onclick="copySupercellLink(this)" data-link="<?=e($op['access_link'])?>">📋 Copy Link</button></div><div class="mini-note">✅ Payment completed — এই protected link শুধু আপনার account-এর জন্য।</div></div><?php endforeach; endif; ?></div>
...
function init(){let s=document.getElementById('languageSelect');if(!s)return;let l=localStorage.getItem('trustedBazaarLanguage')||'bn';s.value=L.includes(l)?l:'bn';s.onchange=()=>{localStorage.setItem('trustedBazaarLanguage',s.value);location.reload()};apply(s.value)}document.addEventListener('DOMContentLoaded',init);window.addEventListener('load',()=>apply(localStorage.getItem('trustedBazaarLanguage')||'bn'))})();
</script><script>
(function(){
