...
$minDeposit=(float)($store->setting('min_deposit','50')??'50');if($minDeposit<=0)$minDeposit=50.0;$usdtRate=125.0;$paymentNumbers=['bKash'=>$store->setting('payment_bkash','01782242264')??'01782242264','Nagad'=>$store->setting('payment_nagad','01887928771')??'01887928771','Binance UID'=>'828400982'];
...
if($_SERVER['REQUEST_METHOD']==='POST'){
 ...
 if($action==='save_min_deposit'){needAdmin();$newMin=(float)($_POST['min_deposit']??0);if($newMin<=0){flash('Minimum Deposit 0-এর বেশি হতে হবে।');go('?page=admin#deposit-settings');}$store->setSetting('min_deposit',number_format($newMin,2,'.',''));flash('Minimum Deposit সফলভাবে আপডেট হয়েছে।');go('?page=admin#deposit-settings');}
...
?><!doctype html><html lang="bn"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Trusted Bazaar</title><style>
...
<?php elseif($page==='deposit'):needLogin();?><div class="card form"><h2>Deposit Request</h2><div class="payment-box"><div><b>bKash</b><div class="payment-number"><?=e($paymentNumbers['bKash'])?></div></div><div><b>Nagad</b><div class="payment-number"><?=e($paymentNumbers['Nagad'])?></div></div><div><b>Binance UID</b><div class="payment-number"><?=e($paymentNumbers['Binance UID'])?></div><div class="muted">USDT রেট: <b>125৳ = $1 USDT</b></div></div><div class="muted">Payment করার পর Transaction ID দিন। সর্বনিম্ন Deposit: <b>৳<?=number_format($minDeposit,2)?></b></div></div><form method="post"><input type="hidden" name="csrf" value="<?=e(csrf())?>"><input type="hidden" name="action" value="deposit"><label>Payment Method</label><select name="method" id="paymentMethod" required><option value="bKash">bKash — <?=e($paymentNumbers['bKash'])?></option><option value="Nagad">Nagad — <?=e($paymentNumbers['Nagad'])?></option><option value="Binance UID">Binance UID — <?=e($paymentNumbers['Binance UID'])?></option></select><label>Amount (BDT)</label><input class="input" id="depositAmount" type="number" min="<?=e(number_format($minDeposit,2,'.',''))?>" step="0.01" name="amount" required><div class="muted" id="usdtHint">Minimum deposit ৳<?=number_format($minDeposit,2)?></div><label>Transaction ID</label><input class="input" name="trx_id" required><label>Note</label><textarea class="input" name="note"></textarea><button class="btn green">Submit Deposit</button></form></div><script>(function(){const m=document.getElementById('paymentMethod'),a=document.getElementById('depositAmount'),h=document.getElementById('usdtHint');function u(){const v=parseFloat(a.value||0);const minDeposit=<?=json_encode($minDeposit)?>;h.textContent=m.value==='Binance UID'?(v>=minDeposit?'USDT হিসাব: $'+(v/125).toFixed(2)+' • রেট 125৳ = $1 USDT':'USDT রেট: 125৳ = $1 USDT'):'Minimum deposit ৳'+minDeposit.toFixed(2)}if(m)m.addEventListener('change',u);if(a)a.addEventListener('input',u);u()})();</script>
...
<h3 class="section-title" id="deposit-settings">💳 Minimum Deposit Settings</h3><div class="card"><div class="mini-note" style="margin-top:0;margin-bottom:14px">এখান থেকে Customer-এর সর্বনিম্ন Deposit amount পরিবর্তন করুন। নতুন amount save করার পর Customer Panel-এর validation, input minimum এবং message-এ সেটি automatically ব্যবহার হবে।</div><form method="post" class="settings-grid"><input type="hidden" name="csrf" value="<?=e(csrf())?>"><input type="hidden" name="action" value="save_min_deposit"><div><label>Minimum Deposit (BDT)</label><input class="input" type="number" name="min_deposit" min="0.01" step="0.01" value="<?=e(number_format($minDeposit,2,'.',''))?>" required><div class="small">যেমন: 20, 50, 100</div></div><div style="display:flex;align-items:end"><button class="btn green" type="submit">💾 Save Minimum Deposit</button></div></form></div>
