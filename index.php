...
function init(){let s=document.getElementById('languageSelect');if(!s)return;let l=localStorage.getItem('trustedBazaarLanguage')||'bn';s.value=L.includes(l)?l:'bn';s.onchange=()=>{localStorage.setItem('trustedBazaarLanguage',s.value);location.reload()};apply(s.value)}function formatMoneyBDT(v,l){v=Number(v)||0;if(l==='en')return '$'+(v/130).toFixed(2);if(l==='hi')return '₹'+(v*102/130).toFixed(2);return '৳'+v.toFixed(2)} function applyCurrency(l){document.querySelectorAll('[data-bdt]').forEach(e=>{e.textContent=formatMoneyBDT(e.getAttribute('data-bdt'),l)});let t=document.getElementById('total');if(t&&t.getAttribute('data-bdt-total')==='1'){let q=Number(document.getElementById('qty')?.value||0),p=Number(document.getElementById('price')?.value||0);t.textContent=formatMoneyBDT(q*p/1000,l)}} document.addEventListener('DOMContentLoaded',init);window.addEventListener('load',()=>{let l=localStorage.getItem('trustedBazaarLanguage')||'bn';apply(l);applyCurrency(l)});window.addEventListener('storage',e=>{if(e.key==='trustedBazaarLanguage'){location.reload()}})})();
</script><script>
(function(){
